...
From a development perspective, there are no differences between [a native module](https://microsoft.github.io/react-native-windows/docs/native-modules) and a native UI component. They are both defined inside a Windows Runtime Component, which exposes a `ReactPackageProvider` that gets registered by the main application via autolinking.
...
The first step is to create a user control which we want to make available to the JavaScript layer via JSX. You can choose to create the control in code or via XAML. In this article, I decided to choose the XAML approach, so right click on the Windows Runtime Component, choose **Add -> New item**, select the **User Control** template and give it a meaningful name, like `InkCanvasControl`.
...
### Enabling the InkCanvas control with React Native for Windows
...
In this scenario, we can use a plain command because we are interacting with the User Control, but we aren't expecting anything in return. The content of the `InkCanvas` will be just saved to a file and that's it.
...
1) A reference to the `InkCanvasControl`, which we get by calling the `findNodeHandle()` method with the reference we have previously stored.
2) The command to execute, which is exposed by the `UIManager.getViewManagerConfig().Commands` function. There are two key concept here to highlight: the parameter we pass to the `getViewManagerConfig()` method is the name of the class which hosts our User Control; the `Commands` property, instead, exposes all our commands using the name we have passed to the `ViewManagerCommand` attribute.
...
Another common inking scenario is the requirement to save the content of the canvas, but not as a file on the device, but as as base64 image. This enables developers to send the image, for example, to a cloud service for further processing.
The APIs we need to use in the native layer to achieve this goal are almost the same as the previous scenario. However, in this case, there's an important difference: it isn't anymore a fire & forget command, but we need to get a result back.
...
The implementation of the command is the same we have seen in the previous section, except that we're  The difference is that, this time, the `InkCanvas` control in JSX exposes also an `onInkSaved` event, that we can subscribe.
In this example, the subscription is handled by the `onGetInk()` function which receives, as parameter, the image in base64 returned by the native module. In this example we're just showing the string in the debugger console, but we could also set it as source of an `Image` control to see it, like in the example below:

![](images/base64string.png)
